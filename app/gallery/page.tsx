...
export default function GalleryPage() {
  ...
  const total = useAppSelector((state) => (state.gallery.items as any)?.meta?.total)
  ...
  const handleNext = () => setPage((prev) => (totalPages && prev < totalPages ? prev + 1 : prev))

  return (
    <div className="min-h-screen bg-background">
      {/* Hero Section */}
      <section className="relative py-20 bg-card text-center">
        <Badge className="mb-6 bg-primary/10 text-primary border-primary/20">Our Work</Badge>
        <h1 className="text-4xl sm:text-5xl font-bold text-card-foreground mb-6">Glam Gallery</h1>
        <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
          Explore our portfolio of stunning hair transformations. From precision cuts to vibrant colors,
          see the artistry and expertise that goes into every style we create.
        </p>
      </section>
      ...
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {galleryLoading ? (
            ...
            <div className="text-center py-12 text-muted-foreground text-lg">No items found.</div>
          ) : (
            <>
