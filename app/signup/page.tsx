...
export default function SignupPage() {
  ...
  useEffect(() => {
    if (roleState?.items?.data?.length > 0) {
      const userRole = roleState.items?.data?.find(
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4 py-8">
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div
          className="absolute bottom-20 right-20 w-24 h-24 bg-primary/10 rounded-full blur-lg animate-float"
          style={{ animationDelay: "1.5s" }}
        ></div>
        <div
          className="absolute top-1/2 right-10 w-16 h-16 bg-accent/5 rounded-full blur-lg animate-float"
          style={{ animationDelay: "3s" }}
        ></div>
        <div
          className="absolute bottom-1/3 left-10 w-20 h-20 bg-primary/5 rounded-full blur-lg animate-float"
